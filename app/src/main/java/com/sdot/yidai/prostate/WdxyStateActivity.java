...
public class WdxyStateActivity extends BaseActivity {
    ...
    private ProductStateVo.WdxydVo wdxydVo;
    private OrderWdxydVo orderWdxydVo;
    Dialog dialog;
    private boolean isMaterials = false;

    private static final String TAG = "WdxyStateActivity";

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_wdxy_state);
        ButterKnife.bind(this);
        title.setText("网点信用贷");
        intent = getIntent();
        intent = getIntent();
        wdxydVo = (ProductStateVo.WdxydVo) intent.getSerializableExtra("wdxydVo");
        ...
        switch (view.getId()) {
            ...
            case R.id.mark_name:
                if (isMaterials) {
                    ...
                    bundle.putSerializable("orderWdxydVo", orderWdxydVo);//序列化
                    startActivity(new Intent(this, ApplyDianDateActivity.class).putExtras(bundle));
                } else {
                    Bundle bundle = new Bundle();
                    bundle.putSerializable("orderWdxydVo", orderWdxydVo);//序列化
    ...
    private void ininMp(String state) {
        switch (wdxydVo.getState()) {
            ...
            case "CREATED":
                ...
                tijiaoTime.setText(orderWdxydVo.getCreatedAt().split("T")[0]);
                shehezhongTime.setText(orderWdxydVo.getLastModifiedAt().split("T")[0]);
                break;
            //有效
            case "ENABLED":
                mp1.setCurrNodeNO(3);
            ...
            case "DENIED":
                ...
                chushentongguoTime.setText(orderWdxydVo.getLastModifiedAt().split("T")[0]);
                tijiaoTime.setText(orderWdxydVo.getCreatedAt().split("T")[0]);
                shehezhongTime.setText(orderWdxydVo.getLastModifiedAt().split("T")[0]);
                break;
            //通过
                ...
                .execute(new StringCallback() {
                    ...
                    public void onSuccess(Response<String> response) {
                        ...
                        try {
                            ...
                            totalMoney.setText(ToolUtils.formatStringNumber(orderWdxydVo.getApplyAmount()));
                            guaranteeMoney.setText(ToolUtils.formatDoubleNumber(Double.valueOf(orderWdxydVo.getApplyAmount() + "") * 0.05));
                            time.setText(orderWdxydVo.getCreatedAt().split("T")[0]);
                            //YDaiApplication.getInstance().setRzzlId(orderWdxydVo.getId() + "");
                            ininMp(wdxydVo.getState());
                            getProductState(orderWdxydVo.getId());
    ...
    private void getProductState(String id) {
        OkGo.<String>get("http://test.edairisk.com/rest/orderwdxyds/" + id + "/state").tag(this)
                .headers("cookie", "session=" + YDaiApplication.getInstance().getCookieValue())
                .execute(new StringCallback() {
                    @Override
                    public void onSuccess(Response<String> response) {

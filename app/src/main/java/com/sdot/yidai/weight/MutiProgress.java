...
public class MutiProgress extends View {
    ...
    Context mContext;
    private static final String TAG = "MutiProgress";
    ...
    public void setCurrNodeState(int currNodeState) {
    ...
    public MutiProgress(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MutiProgress(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        mContext = context;

        TypedArray mTypedArray = context.obtainStyledAttributes(attrs, R.styleable.MutiProgress);
        nodesNum = mTypedArray.getInteger(R.styleable.MutiProgress_nodesNum, 4); //默认一个节点
        ...
        currNodeState = mTypedArray.getInt(R.styleable.MutiProgress_currNodeState, 0);
        currNodeNO = mTypedArray.getInt(R.styleable.MutiProgress_currNodeNO, 0);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        float nodeWidth = ((float) mHeight) / (nodesNum - 1);
        for (int i = 0; i < nodesNum; i++) {
            Node node = new Node();
            if (i == 0)
                node.mPoint = new Point(mWidth / 2 - nodeRadius,(int) nodeWidth * i);
            else if (i == 1)
                node.mPoint = new Point(mWidth / 2 - nodeRadius,((int) nodeWidth * i) - nodeRadius);
            else if (i==2)
                node.mPoint = new Point(mWidth / 2 - nodeRadius,((int) nodeWidth * i) - nodeRadius*2);
            else
                node.mPoint = new Point(mWidth / 2 - nodeRadius,((int) nodeWidth * i) - nodeRadius*3);
        ...
        Log.i(TAG, "onMeasure: "+nodes.size());
    }
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        Log.i(TAG, "onDraw: "+"canvas"+"currNodeNO="+currNodeNO+"==="+"currNodeState="+currNodeState);
        DrawProgerss();
    ...
    private void DrawProgerss() {
       ...
       //mCanvas.drawLine(nodeRadius, mHeight/2, mWidth-nodeRadius, mHeight/2, mPaint);  //线段2端去掉nodeRadius
        mPaint.setColor(Color.parseColor("#00AFEB"));
        mCanvas.drawLine(mWidth/2,nodeRadius,
                nodes.get(currNodeNO).mPoint.x + nodeRadius, nodes.get(currNodeNO).mPoint.y + nodeRadius, mPaint);  //线段2端去掉nodeRadius
        //后半截线段
        mPaint.setColor(Color.parseColor("#dddddd"));
        mCanvas.drawLine(nodes.get(currNodeNO).mPoint.x +nodeRadius, nodes.get(currNodeNO).mPoint.y + nodeRadius,
                mWidth/2,mHeight-2*nodeRadius,  mPaint);  //线段2端去掉nodeRadius
    }
